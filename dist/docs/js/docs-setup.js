NG_DOCS={
  "sections": {
    "api": "API Documentation"
  },
  "pages": [
    {
      "section": "api",
      "id": "componitor",
      "shortName": "componitor",
      "type": "overview",
      "moduleName": "componitor",
      "shortDescription": "Generate simple HTML template -like directives on the fly.",
      "keywords": "angularjs api application block body bootstrap border boxes camelcase center class clear clearfix componitor content contents correctly css ctrl currently democtrl directives display float fly function generate height html ignored ipsum js left loaded lorem margin-left middle module my-box my-box-body my-box-title mybox ng-controller ng-model overview script selector simple solid style table-cell template templates text text-align title type vertical-align width"
    },
    {
      "section": "api",
      "id": "componitor.directive:componitorTemplate",
      "shortName": "componitorTemplate",
      "type": "directive",
      "moduleName": "componitor",
      "shortDescription": "Registers a new html template directive with the given name. The componitor-template is",
      "keywords": "angular angularjs-style api application automatically avoid bootstrapped camelcased componitor componitor-template config contents created dash directive display elements future hidden html method names note parsed problems real registers representation settings style tags template view"
    },
    {
      "section": "api",
      "id": "componitor.directive:content",
      "shortName": "content",
      "type": "directive",
      "moduleName": "componitor",
      "shortDescription": "This element is replaced with the content found by the selector attribute when",
      "keywords": "angular api application attribute bootstrapped browsers class componitor config content contents corresponding directive element elements example final find heading html http jpg kitten-box kitten-box-content kitten-box-heading kittenbox markup method module natively note org parsed queryselector real rendering replaced selector selectors src support supported tags the-content the-heading width wikimedia"
    },
    {
      "section": "api",
      "id": "componitor.service",
      "shortName": "componitor.service",
      "type": "overview",
      "moduleName": "componitor.service",
      "shortDescription": "The module for the template registration service. See the Componitor service",
      "keywords": "api componitor module overview registration service template"
    },
    {
      "section": "api",
      "id": "componitor.service.Componitor",
      "shortName": "componitor.service.Componitor",
      "type": "service",
      "moduleName": "componitor.service",
      "shortDescription": "Parses and registers the directives for the found &lt;componitor-template /&gt;",
      "keywords": "$compile $compileprovider _createdirective _processtemplate add angular angularjs-style api attribute block border call called camelcased class componitor componitor-templates contents create css demomodule directive directive-to-be directives display element elements example find fragment hello-box hello-box-title hellobox html htmltoprocess isolate isolated js method module padding parse parses process processes public registers representation scope script selector service solid styles target template templatehtml templates true usage usages values"
    }
  ],
  "apis": {
    "api": true
  },
  "html5Mode": false,
  "startPage": "api/componitor",
  "scripts": [
    "angular.min.js",
    "componitor.js"
  ]
};